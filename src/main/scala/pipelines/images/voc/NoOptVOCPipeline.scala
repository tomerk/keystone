...
object NoOptVOCPipeline extends Logging {
  ...
  def run(sc: SparkContext, conf: SIFTFisherConfig) {
    ...
    val parsedRDD = VOCLoader(
      ...
      VOCLabelPath(conf.labelPath)).repartition(conf.numParts).persist(StorageLevel.DISK_ONLY)

    val labelGrabber = MultiLabelExtractor andThen
      ClassLabelIndicatorsFromIntArrayLabels(VOCLoader.NUM_CLASSES) andThen MaxClassifier

    val trainingLabels = labelGrabber(parsedRDD)

    // Part 1: Scale and convert images to grayscale.
    val grayscaler = MultiLabeledImageExtractor andThen PixelScaler andThen GrayScaler
    ...
    val pcaTransformer = conf.pcaFile match {
      ...
      case None => {
        ...
        val pca = new DistributedPCAEstimator(conf.descDim).fit(colSampler(pcapipe(grayRDD)))
    ...
    val fisherFeaturizer =  new BensFisherVector(gmm) andThen
    ...
    val model = new LogisticRegressionLBFGSEstimatorNoScaling(regParam = conf.lambda, numFeatures = 2 * conf.descDim * conf.vocabSize).fit(
    ...
    val map = MeanAveragePrecisionEvaluator(testActuals, ClassLabelIndicatorsFromIntLabels(VOCLoader.NUM_CLASSES).apply(predictions.map(_.toInt)), VOCLoader.NUM_CLASSES)
