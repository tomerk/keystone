...
object NoOptTimitPipeline extends Logging {
  ...
  def run(sc: SparkContext, conf: TimitConfig) {
    ...
    // Build the pipeline
    val trainDataAndLabels = timitFeaturesData.train.labels.zip(timitFeaturesData.train.data).repartition(conf.numParts).persist(StorageLevel.DISK_ONLY)
