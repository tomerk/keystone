...
import nodes.learning.{LogisticRegressionLBFGSEstimatorNoScaling, LogisticRegressionLBFGSEstimator}
import nodes.nlp._
...
object NoOptAmazonPipeline extends Logging {
  ...
  def run(sc: SparkContext, conf: AmazonReviewsConfig) {
    ...
    val trainData = LabeledData(AmazonReviewsDataLoader(sc, conf.trainLocation, conf.threshold).labeledData.repartition(conf.numParts).persist(StorageLevel.DISK_ONLY))
    ...
    val featurizedTrainData = featurizer.apply(training)
