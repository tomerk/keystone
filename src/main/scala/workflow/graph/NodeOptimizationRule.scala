 ...
 */
private[graph] class NodeOptimizationRuleGraphExecutor(graph: Graph, state: Map[GraphId, Expression], samplesPerPartition: Int) {

  // The mutable internal state attached to the optimized graph. Lazily computed, requires optimization.
  private val optimizedState: scala.collection.mutable.Map[GraphId, Expression] =
    scala.collection.mutable.Map() ++ state
   ...
   */
  def getState: Map[GraphId, Expression] = optimizedState.toMap
        ...
        case node: NodeId => {
          ...
          val operator = graph.getOperator(node) match {
            ...
            case op: TransformerOperator => {
              numPerPartition(node) = numPerPartition(dependencies(0))
              op
            }
  ...
  // TODO: Add sampling-per-partition
  // TODO: Add num-per-partition tracking (and accessing)
}
...
class NodeOptimizationRule(samplesPerPartition: Int = 3) extends Rule {
  ...
  def apply(graph: Graph, state: Map[GraphId, Expression]): (Graph, Map[GraphId, Expression]) = {
    ...
    val nodesToOptimize = graph.operators.collect {
      case (node, op: OptimizableTransformer) if !descendantsOfSources.contains(node) => node
    ...
    val executor = new NodeOptimizationRuleGraphExecutor(graph, state, samplesPerPartition)
  }
