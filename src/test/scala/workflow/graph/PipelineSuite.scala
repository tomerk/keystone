...
class PipelineSuite extends FunSuite with LocalSparkContext with Logging {
  ...
  test("Do not fit estimators multiple times") {
    ...
    Pipeline.session(Seq(pipelineOut, pipelineOut2), Some(DefaultOptimizer))
  ...
  test("estimator chaining") {
    ...
    Pipeline.session(Seq(pipelineOutOne, pipelineOutTwo, modelOut), None)
  ...
  test("label estimator chaining") {
    ...
    Pipeline.session(Seq(pipelineOutOne, pipelineOutTwo, modelOut, features), None)
